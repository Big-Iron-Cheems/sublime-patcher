...
private:
    ...
    const PatchCollection &getPatches(const SublimeApp &app) const;
    /// Get all patches for Sublime Text for a specific platform and architecture
    const PatchCollection &getSublimeTextPatches(Platform platform, Architecture arch) const;
    /// Get all patches for Sublime Merge for a specific platform and architecture
    const PatchCollection &getSublimeMergePatches(Platform platform, Architecture arch) const;
